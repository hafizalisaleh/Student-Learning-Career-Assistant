...
import {
  ...
  Trophy, RefreshCw, ArrowUp, ArrowDown, Minus, GitBranch, Layers, Sparkles
} from 'lucide-react';
...
export default function ProgressPage() {
  ...
  const loadAnalytics = async () => {
    try {
      ...
      // Fetch evolution data separately (non-blocking)
      try {
        const evoData = await api.getEvolutionTimeline();
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      {evolutionData && evolutionData.total_concepts > 0 && (
        <div className="space-y-3">
          {/* Section Header */}
          <div className="flex items-center gap-2">
            <GitBranch className="w-4 h-4 text-[var(--progress)]" />
            <h3 className="text-sm font-medium text-[var(--text-secondary)]">Knowledge Evolution</h3>
            <span className="text-xs text-[var(--text-muted)] ml-auto">
              {evolutionData.total_concepts} concept{evolutionData.total_concepts !== 1 ? 's' : ''} tracked
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="card p-3 flex items-center gap-3">
              <Layers className="w-4 h-4 text-[var(--progress)]" />
              <div>
                <p className="text-lg font-semibold text-[var(--text-primary)]">{evolutionData.total_concepts}</p>
                <p className="text-xs text-[var(--text-muted)]">Concepts</p>
              </div>
            </div>
            <div className="card p-3 flex items-center gap-3">
              <Sparkles className="w-4 h-4 text-[var(--notes)]" />
              <div>
                <p className="text-lg font-semibold text-[var(--text-primary)]">
                  {(() => {
                    const levels = evolutionData.concepts?.map((c: any) => c.concept?.current_level || 1) || [];
                    return levels.length ? (levels.reduce((a: number, b: number) => a + b, 0) / levels.length).toFixed(1) : '—';
                  })()}
                </p>
                <p className="text-xs text-[var(--text-muted)]">Avg Level</p>
              </div>
            </div>
            <div className="card p-3 flex items-center gap-3">
              <Brain className="w-4 h-4 text-[var(--summaries)]" />
              <div>
                <p className="text-lg font-semibold text-[var(--text-primary)] truncate">
                  {Object.keys(evolutionData.domain_summary || {})[0] || '—'}
                </p>
                <p className="text-xs text-[var(--text-muted)]">Top Domain</p>
              </div>
            </div>
          </div>
          ...
          {/* Timeline Chart */}
          {(() => {
            // Build chart data from top 5 concepts with timeline points
            const conceptsWithTimeline = (evolutionData.concepts || [])
              .filter((c: any) => c.timeline && c.timeline.length > 0)
              .sort((a: any, b: any) => (b.concept?.current_level || 0) - (a.concept?.current_level || 0))
              .slice(0, 5);

            if (conceptsWithTimeline.length === 0) return null;

            // Collect all unique dates
            const dateSet = new Set<string>();
            conceptsWithTimeline.forEach((c: any) => {
              c.timeline.forEach((t: any) => {
                if (t.date) dateSet.add(t.date.split('T')[0]);
              });
            });
            const dates = Array.from(dateSet).sort();

            // Build chart data: each date has a level value per concept
            const chartData = dates.map(date => {
              const point: any = {
                date: new Date(date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })
              };
              conceptsWithTimeline.forEach((c: any) => {
                const name = c.concept?.canonical_name || 'Unknown';
                const snap = c.timeline.filter((t: any) => t.date?.startsWith(date)).pop();
                point[name] = snap ? snap.level : null;
              });
              return point;
            });

            const EVOLUTION_COLORS = ['#06B6D4', '#5C8A72', '#B8860B', '#7C6B8E', '#4B6A9B'];

            return (
              <div className="card p-4">
                <h3 className="text-sm font-medium text-[var(--text-secondary)] mb-3">Concept Level Progression</h3>
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={chartData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--card-border)" />
                    <XAxis dataKey="date" stroke="var(--text-muted)" fontSize={11} />
                    <YAxis domain={[0, 5]} ticks={[1, 2, 3, 4, 5]} stroke="var(--text-muted)" fontSize={11} />
                    <Tooltip
                      contentStyle={{
                        backgroundColor: 'var(--card-bg)',
                        border: '1px solid var(--card-border)',
                    ...
                    />
                    <Legend wrapperStyle={{ fontSize: '11px' }} />
                    {conceptsWithTimeline.map((c: any, i: number) => (
                      <Line
                        key={c.concept?.id || i}
                        type="stepAfter"
                        dataKey={c.concept?.canonical_name || 'Unknown'}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {(evolutionData.concepts || [])
              ...
              .map((item: any) => {
                ...
                const levelColor =
                  c.current_level >= 4 ? 'var(--success)' :
                  c.current_level >= 3 ? 'var(--progress)' :
                  c.current_level >= 2 ? 'var(--warning)' :
                  'var(--text-muted)';
                  ...
                  <div key={c.id} className="card p-3">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="text-sm font-medium text-[var(--text-primary)] truncate">{c.canonical_name}</span>
                        {c.domain && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--bg-secondary)] text-[var(--text-muted)] whitespace-nowrap">
                            {c.domain}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="space-y-1">
                      {[
                        { key: 'exposure', label: 'Exposure', color: '#4B6A9B' },
                        { key: 'depth', label: 'Depth', color: '#5C8A72' },
                        { key: 'notes', label: 'Notes', color: '#B8860B' },
                        { key: 'quiz', label: 'Quiz', color: '#7C6B8E' },
                        { key: 'breadth', label: 'Breadth', color: '#06B6D4' },
                      ].map(({ key, label, color }) => (
                        <div key={key} className="flex items-center gap-2">
                          <span className="text-[10px] text-[var(--text-muted)] w-12">{label}</span>
                          <div className="flex-1 bg-[var(--bg-tertiary)] rounded-full h-1">
                            <div
                              className="h-1 rounded-full transition-all"
                              style={{
                                width: `${(signals[key] || 0) * 100}%`,
          ...
          {/* Domain Summary */}
          {Object.keys(evolutionData.domain_summary || {}).length > 1 && (
            <div className="card p-4">
              <h3 className="text-sm font-medium text-[var(--text-secondary)] mb-3">Domain Overview</h3>
              <div className="space-y-2">
                {Object.entries(evolutionData.domain_summary || {})
                  .sort((a: any, b: any) => b[1].avg_level - a[1].avg_level)
                  .map(([domain, info]: [string, any]) => (
                    <div key={domain} className="flex items-center justify-between">
                      <div className="flex items-center gap-2 flex-1">
                        <span className="text-sm text-[var(--text-primary)]">{domain}</span>
                        <span className="text-xs text-[var(--text-muted)]">
                          {info.concept_count} concept{info.concept_count !== 1 ? 's' : ''}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="w-20 bg-[var(--bg-tertiary)] rounded-full h-1.5">
                          <div
                            className="h-1.5 rounded-full bg-[var(--progress)]"
                            style={{ width: `${(info.avg_level / 5) * 100}%` }}
                          />
                        </div>
