    ...
    import("mind-elixir").then((MindElixirModule) => {
      ...
      try {
        ...
        if (isSubscribed) {
          ...
          // Listen for multi-select via area selection
          ...
          // Listen for single-click on nodes (selectNodes event doesn't fire for single clicks)
          if (onSelectNodesRef.current) {
            const handleNodeClick = (e: MouseEvent) => {
              const target = e.target as HTMLElement;
              // me-tpc is the MindElixir topic element tag
              const tpc = target.closest('me-tpc');
              if (!tpc) return;
              // Walk up to find the wrapper with data-nodeid (set by MindElixir as "me" + id)
              const wrapper = tpc.closest('[data-nodeid]') as HTMLElement | null;
              // MindElixir stores nodeObj directly on the DOM element
              const nodeObj = (wrapper as any)?.nodeObj || (tpc.parentElement as any)?.nodeObj;
              if (nodeObj) {
                onSelectNodesRef.current?.([nodeObj]);
              } else {
                // Fallback: build minimal NodeObj from DOM text
                const topic = tpc.textContent?.trim() || '';
                if (topic) {
                  const nodeId = wrapper?.dataset?.nodeid?.replace(/^me/, '') || topic;
                  onSelectNodesRef.current?.([{ id: nodeId, topic } as NodeObj]);
                }
              }
            };
